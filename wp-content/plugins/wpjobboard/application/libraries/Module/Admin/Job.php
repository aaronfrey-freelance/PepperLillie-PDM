...
class Wpjb_Module_Admin_Job extends Wpjb_Controller_Admin
{
    ...
    public function indexAction()
    {
        ...
        foreach(array("all", "active", "unread", "expired", "expiring", "awaiting", "inactive") as $f) {
            $stat->$f = Wpjb_Model_JobSearch::search(array(
                "filter"=>$f,
                "count_only"=>1
            ));
        }
        ...
        if($this->_request->get("employer")) {
            $query["employer_id"] = intval($this->_request->get("employer"));
            $param["employer"] = $this->_request->get("employer");
            $this->view->employer = $query["employer_id"];
            $this->view->company = new Wpjb_Model_Company($query["employer_id"]); 
        }
