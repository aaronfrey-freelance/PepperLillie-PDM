...
class Wpjb_Form_Admin_Config_Linkedin extends Daq_Form_Abstract
{
    ...
    public function executeInit($controller = null)
    {
        ...
        if(!$linkedin->getOauthToken() && $request->get("do") == "get-access-token") {
            ...
            $tokenUrl = $linkedin->requestToken(array("redirect_url"=>$url));
    ...
    public function executePostSave($controller = null)
    {
        ...
        if($linkedin_api_key && $linkedin_oauth_token) {
            ...
            $e->setValue($instance->getConfig("linkedin_apply_method", "url"));
            $this->addElement($e, "apply");
            
            $e = $this->create("linkedin_apply_email");
            $e->setLabel(__("Email", "wpjobboard"));
            $e->setHint(__("Used only if you wish to receive LinkedIn applications by email only.", "wpjobboard"));
            $e->setValue($instance->getConfig("linkedin_apply_email"));
            $this->addElement($e, "apply");
       }
