...
class Wpjb_Project extends Daq_ProjectAbstract
{
    ...
    public function run()
    {
        ...
        add_action("wp", array("Wpjb_Utility_Yoast", "connect"), 20);
        add_action("wp", array("Wpjb_Utility_Genesis", "connect"), 20);
        ...
        if(isset($linkedin_apply[0]) && $linkedin_apply[0]==1) {
            add_action("wpjb_tpl_single_actions", array("Wpjb_Service_Linkedin", "apply"));
        }
    ...
    {   
        ...
        wp_register_script('wpjb-js', plugins_url().'/wpjobboard/templates/js.js', array("jquery"), self::VERSION);
        wp_register_style("wpjb-css", plugins_url()."/wpjobboard/templates/style.css", array(), self::VERSION);
